                      ...
                      std::vector<cv::DMatch> &matches, std::string descriptorCategory, std::string matcherType, std::string selectorType)
{
    ...
    // Perform matching task (nearest neighbor matching (best match))
    if (selectorType.compare("SEL_NN") == 0)
    {
        matcher->match(descSource, descRef, matches);
    }
    
    // Perform matching task (k nearest neighbors (k=2))
...
void detKeypointsShiTomasi(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    int blockSize = 4;                                              // avg block size for computing a derivative covariation matrix over each pixel neighborhood
...
// Detect keypoints in an image by using the Harris detector developed earlier
void detKeypointsHarris(vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    // Detector parameters
    int blockSize = 2;              // Neighborhood blocksize 
    int apertureSize = 3;           // Aperture parameter for the Sobel operator
    assert(1 == apertureSize % 2);  // Check for Aperture size to be odd
    ...
    for (size_t j = 0; j < dst_norm.rows; j++) {
        for (size_t i = 0; i < dst_norm.cols; i++) {
            ...
            // Applying minimum threshold for Harris cornerness response
            if (response < minResponse) continue;

            // else create a tentative new keypoint
            cv::KeyPoint newKeyPoint;
            ...
            bool bOverlap = false;
            // Looping over all the existing keypoints
