...
int main(int argc, const char *argv[])
{
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        // bounding box to keep only keypoints on the preceding vehicle
        ...
        if (bFocusOnVehicle)
        {
            ...
            for (auto kp : keypoints) {
                if (vehicleRect.contains(kp.pt)) filteredKeypoints.push_back(kp);  // will retain all the points in here
            }
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            /* Select either Brute Force (BF) or Fast Library for Approximate Nearest Neighbors (FLANN) */
            string matcherType = "MAT_BF";
            // string matcherType = "MAT_FLANN";
            
            /* BINARY descriptors include: BRISK, BRIEF, ORB, FREAK, and AKAZE*/
            /* HOG descriptors include: SIFT */
